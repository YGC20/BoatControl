...
- 전환 과정에서 실제로 쓸 수 있는 보드가 라즈베리파이가 아니라 징크보드(Zynq, PetaLinux)로 정해지면서, 노트북(영상 처리)과 징크보드(모터 제어)로 역할을 나누는 2-장비 구조로 다시 설계했습니다. 이후 vision은 Windows/Linux 양쪽에서 빌드되도록 소켓·빌드 스크립트를 맞췄습니다.
...
- **실시간 객체 탐지 (vision, C++)** — "웹캠 프레임을 캡처해 ONNX Runtime으로 YOLOv8n 모델 추론을 수행하고, 탐지 결과를 매 프레임 UDP로 전송"합니다. ONNX Runtime 없이 빌드하면 stub 탐지기로 동작합니다. 커스텀 학습 모델은 아직 없어 사전학습 COCO 모델로 파이프라인만 검증한 상태입니다.
...
- **모터 제어 (boat, C++)** — 캐릭터 디바이스 커널 모듈(`motor_driver.c`)이 PS MIO GPIO를 직접 제어하고(디바이스 close 시 자동 정지), 유저스페이스 앱(`motor_controller_dev.cpp`)은 `ioctl()`로 방향 명령만 전달합니다. 실물 없이 콘솔 로그만 출력하는 stub 백엔드도 함께 구현해 로직을 먼저 검증할 수 있게 했습니다.
...
- **주행 판단 로직 (boat, C++)** — 초음파 거리가 60cm 미만이면 정지하고, 그렇지 않으면 "탐지된 대상 중 confidence가 가장 높은 하나를 골라 프레임 중심과의 x좌표 차이를 비교해 전진/좌회전/우회전을 결정"하고, 탐지 결과가 없으면 탐색 동작으로 전환합니다.
...
- **커널 모듈 빌드/배포 인프라** — `motor_driver`/`echo_driver` 두 커널 모듈의 `Makefile`(`CROSS_COMPILE`/`KDIR` 경로 — 현재 저장소에는 포함돼 있지 않음, `.gitignore`가 `Makefile`을 제외함)과 `/dev` 노드 접근 권한(udev rule) 설정이 아직 남아있고, 새 구조로 크로스컴파일을 실제 시도해보지는 않은 상태입니다.
...
```
[노트북 · Windows·Linux/C++]                              [징크보드 · PetaLinux/C++]
   vision/                                              boat/
   웹캠 캡처                                              초음파 센서 확인
...
| 장치 간 통신 | UDP 소켓 (vision: WinSock / POSIX, boat: POSIX) | `DetectionPacket` 구조체 그대로 송수신 |
| 하드웨어 제어 (boat) | 커널 캐릭터 디바이스 드라이버(ioctl/interrupt) + libgpiod | 모터 방향 제어(ioctl), 초음파 trigger(libgpiod)·echo 인터럽트(커널 모듈) |
| 빌드 시스템 | CMake(vision, boat/app 각각 독립) + 커널 모듈 Makefile | `USE_ORT`(vision), `USE_HW_MOTOR`/`USE_HW_DISTANCE`(boat) 옵션으로 stub ↔ 실장치 전환 |
...
│  │     ├─ motor_driver/           # motor_driver.c, motor_ioctl.h (Makefile은 미포함)
│  │     └─ echo_driver/            # echo_driver.c (Makefile은 미포함)
...
│  ├─ distance_sensor.py
│  └─ button_handler.py
├─ model/                           # (저장소 미포함) yolov8n.onnx를 직접 넣어야 함 — *.onnx는 .gitignore 대상
├─ document/
│  ├─ report/                       # 진행 기록 (report_YYYYMMDD.md)
...
- **vision (C++)**: 웹캠 캡처 → YOLOv8n(ONNX Runtime) 추론 → UDP 송신까지 빌드 및 동작 확인 완료(Windows에서 ONNX Runtime 포함 검증, Linux는 소켓·CMake를 맞춘 단계로 `USE_ORT`의 ONNX Runtime 링크 경로는 아직 Windows용 `.lib`/`.dll` 기준). `--host`만 CLI 인자로 처리되고 `--port`/`--camera`는 아직 기본값 고정입니다.
- **boat (C++)**: 기존 libgpiod 유저스페이스 방식에서 커널 캐릭터 디바이스 드라이버(`motor_driver`, `echo_driver`) + ioctl/interrupt 기반 구조로 재설계했습니다. 모터 제어(ioctl/stub), 초음파 trigger+인터럽트 기반 거리 측정, UDP 수신, 제어 루프까지 코드는 모두 작성했지만, 커널 모듈 `Makefile` 작성(저장소 미포함)과 크로스 툴체인 경로 설정, 실제 크로스컴파일은 새 구조로 아직 시도해보지 않은 상태입니다.
...
2. `motor_driver`/`echo_driver` 커널 모듈 `Makefile` 작성(스페이스 대신 탭 들여쓰기 주의, `.gitignore`의 `Makefile` 규칙 예외 처리) 후 `CROSS_COMPILE`/`KDIR` 경로를 PetaLinux 프로젝트에 맞게 채우고 빌드 검증
...
9. `vision/CMakeLists.txt`의 ONNX Runtime 링크를 Linux(`.so`)에서도 동작하도록 확장
